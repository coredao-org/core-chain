...
func (s *Snapshot) updateAttestation(header *types.Header, chainConfig *params.ChainConfig, satoshiConfig *params.SatoshiConfig) {
	if !chainConfig.IsLuban(header.Number) {
		return
	}

	// The attestation should have been checked in verify header, update directly
	attestation, _ := getVoteAttestationFromHeader(header, chainConfig, satoshiConfig)
	if attestation == nil {
		return
	}
...
func parseValidators(header *types.Header, chainConfig *params.ChainConfig, satoshiConfig *params.SatoshiConfig) ([]common.Address, []types.BLSPublicKey, error) {
	validatorsBytes := getValidatorBytesFromHeader(header, chainConfig, satoshiConfig)
